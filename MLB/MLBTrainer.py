...
database_url = 'mssql+pyodbc://danny1phantom:Popp151565__@yoffsornah.database.windows.net:1433/YoffsOrNah-train-MLB?driver=ODBC+Driver+18+for+SQL+Server'
engine = create_engine(database_url)

# Load Data
query = "SELECT * FROM [dbo].[Baseball-Training-Stats]"
...
with engine.begin() as conn:
    ...
    for index, row in pred_train_df.iterrows():
        ...
        result = conn.execute(
            text("SELECT COUNT(*) FROM [dbo].[Baseball-Predict-Stats] WHERE team = :team AND year = :year"),
            {'team': team, 'year': year}
        ).fetchone()
        ...
        if result[0] > 0:
            ...
            conn.execute(
                text("UPDATE [dbo].[Baseball-Predict-Stats] SET yoffs_pred = :yoffs_pred WHERE team = :team AND year = :year"),
            ...
            # Insert new row
            conn.execute(
                text("INSERT INTO [dbo].[Baseball-Predict-Stats] (year, team, yoffs_pred) VALUES (:year, :team, :yoffs_pred)"),
