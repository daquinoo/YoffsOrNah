...
hotstreak_db_url = 'mssql+pyodbc://danny1phantom:{pwd}@yoffsornah.database.windows.net:1433/YoffsOrNah-HotStreak-ChampNHL?driver=ODBC+Driver+18+for+SQL+Server'
hotstreak_engine = create_engine(hotstreak_db_url)

# Load Data
query_stats = "SELECT * FROM [dbo].[Hockey-Stats] WHERE year >= 2016 AND year <= 2024"
df = pd.read_sql(query_stats, hotstreak_engine)

# Define relevant columns for features and target

feature_columns = [
    'win_pct', 'conf_stdg', 'lg_rnk', 'pts_per_game', 'goalsag_pg', 'goalsfor_pg', 'shots_og_pg',
    'shots_fc_pg', 'shg_for_pg','shg_ag_pg', 'powerplay_ops', 'pp_ops_against', 'yoffs_pred'
]

invert_features = [
    'conf_stdg', 'lg_rnk', 'goalsag_pg', 'shg_ag_pg', 'shots_fc_pg', 'pp_ops_against'
]

differential_columns = [
    'win_pct', 'conf_stdg', 'lg_rnk', 'pts_diff', 'goals_diff',
    'shots_diff', 'pp_diff', 'shg_diff', 'yoffs_pred'
]
...
def calculate_differentials(averages):
    differentials = {
        ...
        'lg_rnk': averages[2],
        'pts_diff': averages[3],
        'goals_diff': averages[5] - averages[4],
        'shots_diff': averages[6] - averages[7],
        'pp_diff': averages[8] - averages[9],
        'shg_diff': averages[10] - averages[11],
        'yoffs_pred': averages[12]
    }
...
def split_data(y_column): # fix split data
    ...
    if y_column == 'yoffs_rdone':
        ...
        playoff_teams = train_df[train_df[y_column].notnull()].groupby(['team', 'year']).nth(3).reset_index()[['team', 'year']] # need to fix this too to make this adjust to first second third fourth instance
...
make_predictions(list(range(2016, 2024)))
