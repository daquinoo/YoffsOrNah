...
def update_database(data, year=2023):  # Assuming year is 2023 if not specified
    for entry in data:
        try:
            cursor.execute("""
...
def main():
    url = "https://www.pro-football-reference.com/years/2023/#all_kicking_stats"
    html_content = fetch_html(url)
    if html_content:
        kicking_data = parse_kicking_stats(html_content)
        if kicking_data:
            update_database(kicking_data)
            print("Kicking data updated successfully.")
        else:
