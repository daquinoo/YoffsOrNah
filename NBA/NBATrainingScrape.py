...
def parse_div_standings(html_content, table_id):
    soup = BeautifulSoup(html_content, 'html.parser')
    table = soup.find('table', id=table_id)
    standings = {}
    if table:
...
def assign_conf_stdg(standings):
    sorted_teams = sorted(standings.items(), key=lambda item: item[1]['win_pct'], reverse=True)
    for rank, (team, data) in enumerate(sorted_teams, start=1):
        standings[team]['conf_stdg'] = rank
    return standings
...
def insert_data(data, year):
    for entry in data:
        entry['year'] = year  # Assign the correct year
        try:
            cursor.execute("""
                INSERT INTO [dbo].[Basketball-Training-Stats] (year, team, games_played, win_pct, conf_stdg, lg_rank,
...
def main():
    for year in range(2024, 2024+1):
        ...
        try:
            ...
            if html_content:
                if 'divs_standings_E' in html_content and 'divs_standings_W' in html_content:
                    east_standings = parse_div_standings(html_content, 'divs_standings_E')
                    west_standings = parse_div_standings(html_content, 'divs_standings_W')
                    east_standings = assign_conf_stdg(east_standings)
                    west_standings = assign_conf_stdg(west_standings)
