...
def parse_defense_stats(html_content):
    ...
    if table:
        ...
        for row in last_five_regular_season_games:
            game_log = {
                'err_pg': get_numeric_value(row.find('td', {'data-stat': 'errors'})),
                'tack_int_pg': get_numeric_value(row.find('td', {'data-stat': 'tackles_interceptions'})),
                'tack_per': get_numeric_value(row.find('td', {'data-stat': 'challenge_tackles_pct'})),
                'shots_blocked_pg': get_numeric_value(row.find('td', {'data-stat': 'blocked_shots'}))
            }
        ...
        for row in rows[last_regular_season_game_index:]:
            ...
            if start_collecting:
                err_pg_cell = row.find('td', {'data-stat': 'errors'})
                tack_int_pg_cell = row.find('td', {'data-stat': 'tackles_interceptions'})
                tack_per_cell = row.find('td', {'data-stat': 'challenge_tackles_pct'})
                shots_blocked_pg_cell = row.find('td', {'data-stat': 'blocked_shots'})
                if err_pg_cell and tack_int_pg_cell and tack_per_cell and shots_blocked_pg_cell:
                    game_log = {
                        'err_pg': get_numeric_value(err_pg_cell),
                        'tack_int_pg': get_numeric_value(tack_int_pg_cell),
                        'tack_per': get_numeric_value(tack_per_cell),
                        'shots_blocked_pg': get_numeric_value(shots_blocked_pg_cell)
                    }
                    game_logs.append(game_log)
    return game_logs
...
def update_game_logs(year, team_name, game_logs):
    for i, game_log in enumerate(game_logs):
        cursor.execute("""
            ...
            SET err_pg = ?, tack_int_pg = ?, tack_per = ?, shots_blocked_pg = ?
            WHERE year = ? AND team = ? AND games_played = ?
        """, (
            float(game_log['err_pg']), 
            float(game_log['tack_int_pg']), 
            float(game_log['tack_per']), 
            float(game_log['shots_blocked_pg']), 
            year, team_name, i + 1
        ))
    conn.commit()
...
def main():
    ...
    standings_tables = [
        'results2023221Eastern-Conference_overall', 
        'results2023221Western-Conference_overall'
    ]
    ...
    for team_name in standings:
        ...
        if team_code:
            team_url = f"https://fbref.com/en/squads/{team_code}/{year}/matchlogs/c22/defense/{team_name.replace(' ', '-')}-Match-Logs-Major-League-Soccer"
            html_content = fetch_html(team_url)
            game_logs = parse_defense_stats(html_content)
            update_game_logs(year, team_name, game_logs)
