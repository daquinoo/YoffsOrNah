...
hotstreak_db_url = 'mssql+pyodbc://danny1phantom:Popp151565__@yoffsornah.database.windows.net:1433/YoffsOrNah-HotStreak-ChampMLS?driver=ODBC+Driver+18+for+SQL+Server'
hotstreak_engine = create_engine(hotstreak_db_url)

# Load Data
query_stats = "SELECT * FROM [dbo].[Soccer-Stats] WHERE year >= 2018 AND year <= 2023"
...
feature_columns = [
    'win_pct', 'conf_stdg', 'lg_rnk', 'pts_per_game', 'gf_pg', 'ga_pg', 'save_per', 
    'shots_ot_pg', 'cmp_pass_prc', 'poss_prc', 'sca_pg', 'err_pg', 'tack_int_pg', 
    'tack_per', 'shots_blocked_pg', 'takeon_perc', 'keypass_pg', 'aerial_perc', 
    'cross_pg', 'setp_g_pg', 'owng_conc_pg', 'looseballs_rec_pg', 'pkwon_pg', 
    'pkconc_pg', 'fls_pg', 'fld_pg', 'yoffs_pred'
]

invert_features = [
    'conf_stdg', 'lg_rnk', 'ga_pg', 'err_pg', 'owng_conc_pg', 
    'pkconc_pg', 'fls_pg'
]

differential_columns = [
    'win_pct', 'conf_stdg', 'lg_rnk', 'pts_diff', 'goals_diff', 
    'shots_diff', 'cmp_pass_diff', 'poss_diff', 'sca_diff', 
    'err_diff', 'tack_int_diff', 'tack_eff', 'block_diff', 
    'takeon_diff', 'keypass_diff', 'aerial_diff', 'cross_diff', 
    'setp_g_diff', 'own_goal_diff', 'looseballs_diff', 'pk_diff', 
    'foul_diff', 'fld_diff', 'yoffs_pred'
]
...
def calculate_differentials(averages):
    differentials = {
        ...
        'lg_rnk': averages[2],
        'pts_diff': averages[3],
        'goals_diff': averages[4] - averages[5],
        'shots_diff': averages[6],
        'cmp_pass_diff': averages[7],
        'poss_diff': averages[8],
        'sca_diff': averages[9],
        'err_diff': averages[10],
        'tack_int_diff': averages[11],
        'tack_eff': averages[12],
        'block_diff': averages[13],
        'takeon_diff': averages[14],
        'keypass_diff': averages[15],
        'aerial_diff': averages[16],
        'cross_diff': averages[17],
        'setp_g_diff': averages[18],
        'own_goal_diff': averages[19],
        'looseballs_diff': averages[20],
        'pk_diff': averages[21] - averages[22],
        'foul_diff': averages[24] - averages[23],
        'yoffs_pred': averages[25]
    }
...
def get_playoff_rolling_averages(df, y_column):
   #if y_column == 'yoffs_wc':
    #    playoff_teams = df[df[y_column].isin([1])].groupby(['team', 'year']).nth(0).reset_index()[['team', 'year']]
    if y_column == 'yoffs_rdone':
        playoff_teams = df[df[y_column].isin([1])].groupby(['team', 'year']).nth(1).reset_index()[['team', 'year']]
    elif y_column == 'yoffs_rtwo':
        playoff_teams = df[df[y_column].isin([1])].groupby(['team', 'year']).nth(2).reset_index()[['team', 'year']]
    elif y_column == 'yoffs_rdthr':
        playoff_teams = df[df[y_column].isin([1])].groupby(['team', 'year']).nth(3).reset_index()[['team', 'year']]
    elif y_column == 'yoffs_champ':
        playoff_teams = df[df[y_column].isin([1])].groupby(['team', 'year']).nth(4).reset_index()[['team', 'year']]
    playoff_averages = []
    ...
    for _, row in playoff_teams.iterrows():
        ...
        playoff_game = team_data[team_data[y_column].isin([1])].iloc[0]
         ...
         #   playoff_game = team_data[team_data[y_column].isin([1])].iloc[0]
        if y_column == 'yoffs_rdone':
            playoff_game = team_data[team_data[y_column].isin([1])].iloc[1]
        elif y_column == 'yoffs_rtwo':
            playoff_game = team_data[team_data[y_column].isin([1])].iloc[2]
        elif y_column == 'yoffs_rdthr':
            playoff_game = team_data[team_data[y_column].isin([1])].iloc[3]
        elif y_column == 'yoffs_champ':
            playoff_game = team_data[team_data[y_column].isin([1])].iloc[4]
    ...
    if isinstance(X_weighted, pd.Series):
        for feature in differential_columns:
            if feature in coefficients:
                mean_coef = coefficients[feature].mean()
                min_coef = coefficients[feature].min()
                max_coef = coefficients[feature].max()
    
                if abs(mean_coef - min_coef) <= abs(max_coef - mean_coef):
                    X_weighted[feature] *= min_coef
                else:
                    X_weighted[feature] *= max_coef
    else:
        for feature in differential_columns:
            if feature in coefficients:
...
'''def split_data(y_column): # fix split data
    def get_team_averages(group):
        ...
        playoff_games = group[group[y_column].notnull()].sort_values('games_played').iloc[index:]
    ...
    print(f"All averages shape: {all_averages.shape}")
    all_averages = pd.DataFrame(all_averages.tolist(), index=all_averages.index, columns=feature_columns)
    
    #if y_column == 'yoffs_wc':
    #    playoff_teams = train_df[train_df[y_column].notnull()].groupby(['team', 'year']).nth(0).reset_index()[['team', 'year']]
    if y_column == 'yoffs_rdone':
        playoff_teams = train_df[train_df[y_column].notnull()].groupby(['team', 'year']).nth(1).reset_index()[['team', 'year']]
    elif y_column == 'yoffs_rtwo':
        playoff_teams = train_df[train_df[y_column].notnull()].groupby(['team', 'year']).nth(2).reset_index()[['team', 'year']]
    elif y_column == 'yoffs_rdthr':
        playoff_teams = train_df[train_df[y_column].notnull()].groupby(['team', 'year']).nth(3).reset_index()[['team', 'year']]
    elif y_column == 'yoffs_champ':
        playoff_teams = train_df[train_df[y_column].notnull()].groupby(['team', 'year']).nth(4).reset_index()[['team', 'year']] # need to fix this too to make this adjust to first second third fourth instance
...
def split_data(y_column): # fix split data
    ...
    print(f"Splitting data for {y_column}")
    all_averages = train_df.groupby(['team', 'year']).apply(get_team_averages)
    print(f"All averages shape: {all_averages.shape}")
    all_averages = pd.DataFrame(all_averages.tolist(), index=all_averages.index, columns=feature_columns)
...
#X_train_wc, X_test_wc, y_train_wc, y_test_wc = split_data('yoffs_wc')
...
#X_train_wc_diff = X_train_wc.apply(calculate_differentials, axis=1)
#X_test_wc_diff = X_test_wc.apply(calculate_differentials, axis=1)
...
#coefficients_wc = individual_regressions(X_train_wc_diff, y_train_wc)
...
#X_weighted_wc = apply_coefficients(X_train_wc_diff, coefficients_wc)
...
#playoff_averages_wc = get_playoff_rolling_averages(train_df, 'yoffs_wc')
...
#playoff_averages_wc = calculate_differentials(playoff_averages_wc)
...
#playoff_averages_wc = apply_coefficients(playoff_averages_wc, coefficients_wc)
...
#X_diff_wc = calculate_differences(X_weighted_wc, playoff_averages_wc)
...
#model_wc = train_model(X_diff_wc, y_train_wc)
    ...
    for year in years:
        ...
        for games_played in range(1, max_games + 1):
            ...
            for team in teams:
                ...
                prev_row = team_data[team_data['games_played'] < games_played].iloc[-1] if games_played > 6 else None
                ...
                if round_to_predict:
                    ...
                    #if round_to_predict == 'yoffs_wc':
                      #  coefficients = coefficients_wc
                     #   playoff_averages = playoff_averages_wc
                    ...
                    for feature, coeff in coefficients.items():
                        if isinstance(coeff, np.ndarray):
                            print(f"{feature}: min = {coeff.min():.4f} mean = {coeff.mean():.4f}, max =  {coeff.max():.4f}")
                        else:
                    ...
                    for feature, avg in zip(feature_columns, rolling_differentials):
                        if feature in coefficients:
                            if isinstance(coefficients[feature], np.ndarray):
                                ...
                                min_coeff = coefficients[feature].min()
                                max_coeff = coefficients[feature].max()
    
                                if abs(mean_coeff - min_coeff) <= abs(max_coeff - mean_coeff):
                                    mean_coeff = min_coeff
                                else:
                                    mean_coeff = max_coeff
                            else:
                    ...
                    #if round_to_predict == 'yoffs_wc':
                     #   prediction = predict_round_advancement(model_wc, wdiff.reshape(1, -1))
                    if round_to_predict == 'yoffs_rdone':
                    ...
                    with hotstreak_engine.begin() as conn:
                        conn.execute(
                            text(f"UPDATE [dbo].[Hockey-Stats] SET {pred_column} = :pred WHERE team = :team AND year = :year AND games_played = :games_played"),
            ...
            if games_played >= 6:
                ...
                for team, pred_info in game_predictions.items():
                    ...
                    if opponent or team in game_predictions and team not in adjusted_predictions:
            ...
            for team, pred_info in game_predictions.items():
                if team in adjusted_predictions:
                    ...
                    team_games_played = adjusted_predictions[team]['games_played']
                else:
                    pred = pred_info['pred']
                ...
                df.loc[(df['team'] == team) & (df['year'] == year) & (df['games_played'] == team_games_played), pred_column] = pred
                with hotstreak_engine.begin() as conn:
                    conn.execute(
                        text(f"UPDATE [dbo].[Hockey-Stats] SET {pred_column} = :pred WHERE team = :team AND year = :year AND games_played = :games_played"),
                        {'pred': pred, 'team': team, 'year': int(year), 'games_played': int(team_games_played)}
                    )
    
                print(f"Prediction Update: Team: {team}, Year: {year}, Games Played: {team_games_played}")
                print(f"Column: {pred_column}, Prediction: {pred:.4f}")

                # Spread prediction to subsequent games only if actual is not null
                if pd.notnull(actual):
                    subsequent_games = df[(df['team'] == team) & (df['year'] == year) & (df['games_played'] > team_games_played)]
        ...
        o_accuracy = accuracy_score(all_actuals, binary_predictions)
        print(f"Overall Accuracy: {o_accuracy:.4f}")
...
# Make predictions for training data (2016-2023)
make_predictions(list(range(2018, 2023)))

# Prepare 2024 data for prediction
test_2024_df = df[df['year'] == 2023]
