...
def parse_goalkeeping_stats(html_content):
    ...
    if table:
        ...
        for row in last_five_regular_season_games:
            game_log = {
                'save_per': get_numeric_value(row.find('td', {'data-stat': 'gk_save_pct'}))
            }
        ...
        for row in rows[last_regular_season_game_index:]:
            ...
            if start_collecting:
                save_per_cell = row.find('td', {'data-stat': 'gk_save_pct'})
                if save_per_cell:
                    game_log = {
                        'save_per': get_numeric_value(save_per_cell)
                    }
                    game_logs.append(game_log)
    return game_logs
...
def main():
    ...
    for team_name in standings:
        ...
        if team_code:
            team_url = f"https://fbref.com/en/squads/{team_code}/{year}/matchlogs/c22/keeper/{team_name.replace(' ', '-')}-Match-Logs-Major-League-Soccer"
            html_content = fetch_html(team_url)
            game_logs = parse_goalkeeping_stats(html_content)
