...
def parse_possession_stats(html_content):
    ...
    if table:
        ...
        for row in last_five_regular_season_games:
            game_log = {
                'poss_prc': get_numeric_value(row.find('td', {'data-stat': 'possession'})),
                'takeon_perc': get_numeric_value(row.find('td', {'data-stat': 'take_ons_won_pct'}))
            }
        ...
        for row in rows[last_regular_season_game_index:]:
            ...
            if start_collecting:
                poss_prc_cell = row.find('td', {'data-stat': 'possession'})
                takeon_perc_cell = row.find('td', {'data-stat': 'take_ons_won_pct'})
                if poss_prc_cell and takeon_perc_cell:
                    game_log = {
                        'poss_prc': get_numeric_value(poss_prc_cell),
                        'takeon_perc': get_numeric_value(takeon_perc_cell)
                    }
                    game_logs.append(game_log)
    return game_logs
...
def update_game_logs(year, team_name, game_logs):
    for i, game_log in enumerate(game_logs):
        cursor.execute("""
            ...
            SET poss_prc = ?, takeon_perc = ?
            WHERE year = ? AND team = ? AND games_played = ?
        """, (
            float(game_log['poss_prc']), 
            float(game_log['takeon_perc']), 
            year, team_name, i + 1
        ))
    conn.commit()
...
def main():
    ...
    for team_name in standings:
        ...
        if team_code:
            team_url = f"https://fbref.com/en/squads/{team_code}/{year}/matchlogs/c22/possession/{team_name.replace(' ', '-')}-Match-Logs-Major-League-Soccer"
            html_content = fetch_html(team_url)
            game_logs = parse_possession_stats(html_content)
