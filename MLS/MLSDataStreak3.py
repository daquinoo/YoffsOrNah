...
def parse_passing_stats(html_content):
    ...
    if table:
        ...
        for row in last_five_regular_season_games:
            game_log = {
                'cmp_pass_prc': get_numeric_value(row.find('td', {'data-stat': 'passes_pct'})),
                'keypass_pg': get_numeric_value(row.find('td', {'data-stat': 'assisted_shots'}))
            }
        ...
        for row in rows[last_regular_season_game_index:]:
            ...
            if start_collecting:
                cmp_pass_prc_cell = row.find('td', {'data-stat': 'passes_pct'})
                keypass_pg_cell = row.find('td', {'data-stat': 'assisted_shots'})
                if cmp_pass_prc_cell and keypass_pg_cell:
                    game_log = {
                        'cmp_pass_prc': get_numeric_value(cmp_pass_prc_cell),
                        'keypass_pg': get_numeric_value(keypass_pg_cell)
                    }
                    game_logs.append(game_log)
    return game_logs
...
def update_game_logs(year, team_name, game_logs):
    for i, game_log in enumerate(game_logs):
        cursor.execute("""
            ...
            SET cmp_pass_prc = ?, keypass_pg = ?
            WHERE year = ? AND team = ? AND games_played = ?
        """, (
            float(game_log['cmp_pass_prc']), 
            float(game_log['keypass_pg']), 
            year, team_name, i + 1
        ))
    conn.commit()
...
def main():
    ...
    for team_name in standings:
        ...
        if team_code:
            team_url = f"https://fbref.com/en/squads/{team_code}/{year}/matchlogs/c22/passing/{team_name.replace(' ', '-')}-Match-Logs-Major-League-Soccer"
            html_content = fetch_html(team_url)
            game_logs = parse_passing_stats(html_content)
