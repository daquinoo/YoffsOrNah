...
def parse_shooting_stats(html_content):
    ...
    if table:
        ...
        for row in last_five_regular_season_games:
            game_log = {
                'win_pct': 1 if row.find('td', {'data-stat': 'result'}).text == 'W' else 0.5 if row.find('td', {'data-stat': 'result'}).text == 'D' else 0,
                'pts_per_game': 3 if row.find('td', {'data-stat': 'result'}).text == 'W' else 1 if row.find('td', {'data-stat': 'result'}).text == 'D' else 0,
                'gf_pg': get_numeric_value(row.find('td', {'data-stat': 'goals_for'})),
                'ga_pg': get_numeric_value(row.find('td', {'data-stat': 'goals_against'})),
                'shots_ot_pg': get_numeric_value(row.find('td', {'data-stat': 'shots_on_target'}))
            }
        ...
        for row in rows[last_regular_season_game_index:]:
            ...
            if start_collecting:
                outcome_cell = row.find('td', {'data-stat': 'result'})
                if outcome_cell and outcome_cell.text:
                    outcome = outcome_cell.text
                    game_log = {
                        'win_pct': 1 if outcome == 'W' else 0.5 if outcome == 'D' else 0,
                        'pts_per_game': 3 if outcome == 'W' else 1 if outcome == 'D' else 0,
                        'gf_pg': get_numeric_value(row.find('td', {'data-stat': 'goals_for'})),
                        'ga_pg': get_numeric_value(row.find('td', {'data-stat': 'goals_against'})),
                        'shots_ot_pg': get_numeric_value(row.find('td', {'data-stat': 'shots_on_target'}))
                    }
...
# Insert game logs in the database
def insert_game_logs(year, team_name, game_logs):
    for i, game_log in enumerate(game_logs):
        cursor.execute("""
            INSERT INTO [dbo].[Soccer-Stats] (
                year, team, games_played, win_pct, pts_per_game, gf_pg, ga_pg, shots_ot_pg, yoffs
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            year, team_name, i + 1, 
            float(game_log['win_pct']), 
            float(game_log['pts_per_game']), 
            float(game_log['gf_pg']), 
            float(game_log['ga_pg']), 
            float(game_log['shots_ot_pg']), 
            1
        ))
    conn.commit()
...
def main():
    ...
    for team_name in standings:
        ...
        if team_code:
            team_url = f"https://fbref.com/en/squads/{team_code}/{year}/matchlogs/c22/shooting/{team_name.replace(' ', '-')}-Match-Logs-Major-League-Soccer"
            html_content = fetch_html(team_url)
            game_logs = parse_shooting_stats(html_content)
