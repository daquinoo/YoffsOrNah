...
def parse_gca_stats(html_content):
    ...
    if table:
        ...
        for row in last_five_regular_season_games:
            game_log = {
                'sca_pg': get_numeric_value(row.find('td', {'data-stat': 'sca'})),
                'setp_g_pg': get_numeric_value(row.find('td', {'data-stat': 'gca_fouled'}))
            }
        ...
        for row in rows[last_regular_season_game_index:]:
            ...
            if start_collecting:
                sca_pg_cell = row.find('td', {'data-stat': 'sca'})
                setp_g_pg_cell = row.find('td', {'data-stat': 'gca_fouled'})
                if sca_pg_cell and setp_g_pg_cell:
                    game_log = {
                        'sca_pg': get_numeric_value(sca_pg_cell),
                        'setp_g_pg': get_numeric_value(setp_g_pg_cell)
                    }
                    game_logs.append(game_log)
    return game_logs
...
def update_game_logs(year, team_name, game_logs):
    for i, game_log in enumerate(game_logs):
        cursor.execute("""
            ...
            SET sca_pg = ?, setp_g_pg = ?
            WHERE year = ? AND team = ? AND games_played = ?
        """, (
            float(game_log['sca_pg']), 
            float(game_log['setp_g_pg']), 
            year, team_name, i + 1
        ))
    conn.commit()
...
def main():
    ...
    for team_name in standings:
        ...
        if team_code:
            team_url = f"https://fbref.com/en/squads/{team_code}/{year}/matchlogs/c22/gca/{team_name.replace(' ', '-')}-Match-Logs-Major-League-Soccer"
            html_content = fetch_html(team_url)
            game_logs = parse_gca_stats(html_content)
